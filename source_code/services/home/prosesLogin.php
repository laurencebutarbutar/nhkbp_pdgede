...
if (!mysqli_connect_errno()){	
	$selectPassword = mysqli_query($con, "SELECT t_users_detail_id, password FROM users where name = '".$usernameLogin."'");
	
	if (mysqli_num_rows($selectPassword) > 0){
        $rowSelectPassword = mysqli_fetch_assoc($selectPassword);
        $password = $rowSelectPassword['password'];
        $usersDetailId = $rowSelectPassword['t_users_detail_id'];

        if(password_verify($passwordLogin, $password) == 1){
            ...
            if (mysqli_num_rows($selectUsersDetail) > 0){
                ...
                $_SESSION['failedLogin'] = "Data Not Found";
                header("Location: ".$pathViewHome."login.php");      
            }            
        }
        else{
            $_SESSION['failedLogin'] = "Wrong Password";
            header("Location: ".$pathViewHome."login.php");
        }
    }
    else{
        $_SESSION['failedLogin'] = "Username Not Exist";
        header("Location: ".$pathViewHome."login.php");
    }    
}
else{
